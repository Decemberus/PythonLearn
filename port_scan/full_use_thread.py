...
def tcpScan(target, port):
    print("tcp全连接扫描 %s with ports %s" % (target, port))
    # for port in ports:
...
def ackScan(target, ports):
    ...
    for port in ports:
        ...
        if (str(type(ack_flag_scan_resp)) == "<class 'NoneType'>"):
            ...
            if (int(ack_flag_scan_resp.getlayer(ICMP).type) == 3 and int(ack_flag_scan_resp.getlayer(ICMP).code) in [1,
                                                                                                                     2,
                                                                                                                     3,
                                                                                                                     9,
                                                                                                                     10,
...
def nullScan(target, ports):
    ...
    for port in ports:
        ...
        if (str(type(null_scan_resp)) == "<class 'NoneType'>"):
            ...
            if (int(null_scan_resp.getlayer(ICMP).type) == 3 and int(null_scan_resp.getlayer(ICMP).code) in [1, 2, 3, 9,
                                                                                                             10, 13]):
                print_ports(port, "Filtered")
...
def finScan(target, ports):
    ...
    for port in ports:
        ...
        if (str(type(fin_scan_resp)) == "<class 'NoneType'>"):
            ...
            if (int(fin_scan_resp.getlayer(ICMP).type) == 3 and int(fin_scan_resp.getlayer(ICMP).code) in [1, 2, 3, 9,
                                                                                                           10, 13]):
                print_ports(port, "Filtered")
...
def mutiprocess(target, ports, method):
    pool = multiprocessing.Pool(3)
    for port in ports:
        pool.apply_async(func=method, args=(target, port))
...
if __name__ == '__main__':
    ...
    # 扫码方式
    if scantype == "T":  # 全连接扫描
        mutiprocess(target,ports,tcpScan)
